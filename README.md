...
This NPM module is composed of 6 functions in total. They are presented right here:
...
* a function to define the UTC offset from the result of dividing the number of seconds in the wished timezone's offset from UTC (Universal coordinated time) - and which can be an integer or a float - by the number of seconds in an hour, which is therefore 3600.
...
openWeather.getWeather(weatherReferencesHashTable, openWeatherAPIKey).then(function(results) {
    ...
    for(i = 0; i < results.length; i++)
    {
        ...
        if(results[0].cod === 200) {
            ...
            console.log("UTC offset: " + openWeather.utcOffsetDefinitionFunction(results[0].utc_offset));
...
weatherReferencesHashTable["Atuona"] = "PF";
weatherReferencesHashTable["Port-aux-Francais"] = "TF";
...
openWeather.getWeather(weatherReferencesHashTable, openWeatherAPIKey).then(function(results) {
    ...
    for(i = 0; i < results.length; i++)
    {
        ...
        if(results[i].cod === 200) {
            ...
            console.log("UTC offset: " + openWeather.utcOffsetDefinitionFunction(results[i].utc_offset));
